...
//   useEffect(() => {
//   if (!libraryReady || !hasLoadedRef.current) {
//     return;
//   }
...
// //       } else {
// //         toast("Email is still not verified");
// //       }

// //       return refreshedUser;
// //     } catch (error) {
// //       toast.error(error.message || "Unable to refresh verification status");
// //       throw error;
// //     }
// //   }
...
//   // Reset so save effect doesn't fire while reloading
...
//   useEffect(() => {
//     if (!libraryReady || !hasLoadedRef.current) return;
...
//     try {
//       if (user?.uid) {
//         localStorage.removeItem(`muzify-library-v2-${user.uid}`);
//       }
...
export function PlayerProvider({ children }) {
  ...
  // Load library once auth is ready, and again on login/logout
//   useEffect(() => {
//     if (!authReady) return;
...
//     loadPersistedLibrary(user).then(({ state, cloudSync }) => {
//       if (!active) return;
//       setLibrary(state);
//       setIsCloudSyncEnabled(cloudSync);
//       setLibraryReady(true);
...
//   // Save on every library change after first load
// useEffect(() => {
//   if (!libraryReady || !hasLoadedRef.current) return;
//   savePersistedLibrary(library, user).then((cloudSync) => {
//     setIsCloudSyncEnabled(cloudSync);
//   });
// }, [library, libraryReady]);
...
useEffect(() => {
  ...
  const rememberTrack = (track) => {
    ...
    setLibrary((prev) => ({
      ...prev,
      history: dedupeTracks([track, ...prev.history]).slice(0, 18),
    }));
  };

  const playQueue = (tracks, startIndex = 0) => {
    if (!tracks?.length) { toast.error("No playable tracks available"); return; }
    const nextQueue = dedupeTracks(tracks);
    const boundedIndex = Math.min(startIndex, nextQueue.length - 1);
    setQueue(nextQueue);
    setCurrentIndex(boundedIndex);
  ...
  const playTrack = (track) => playQueue([track], 0);
  ...
  const playPrevious = () => {
    if (!queue.length || currentIndex === 0) { setPlaybackTime(0); return; }
    const nextIndex = currentIndex - 1;
    setCurrentIndex(nextIndex);
    setPlaybackTime(0);
    setPlaybackDuration(0);
  ...
  const isTrackLiked = (trackId) => library.liked.some((t) => t.id === trackId);

  const toggleLike = (track) => {
    setLibrary((prev) => {
      const alreadyLiked = prev.liked.some((e) => e.id === track.id);
      return {
        ...prev,
        liked: alreadyLiked
          ? prev.liked.filter((e) => e.id !== track.id)
          : dedupeTracks([track, ...prev.liked]),
      };
    });
  };
  ...
  const createPlaylist = (name) => {
    ...
    setLibrary((prev) => ({ ...prev, playlists: [...prev.playlists, playlist] }));
    toast.success("Playlist created");
    return id;
  };
  ...
  const addTracksToPlaylist = (playlistId, tracks) => {
    setLibrary((prev) => ({
      ...
      playlists: prev.playlists.map((pl) =>
        pl.id === playlistId
          ? { ...pl, tracks: dedupeTracks([...pl.tracks, ...tracks]), updatedAt: new Date().toISOString() }
          : pl
      ),
    }));
  };
  ...
  const addTrackToPlaylist = (playlistId, track) => {
    const exists = library.playlists.find((pl) => pl.id === playlistId);
    if (!exists) {
      const nextId = createPlaylist("Quick Capture");
      addTracksToPlaylist(nextId, [track]);
      toast.success("Saved to a new playlist");
  ...
  const renamePlaylist = (playlistId, name) => {
    const nextName = name.trim();
    if (!nextName) { toast.error("Playlist name cannot be empty"); return; }
    setLibrary((prev) => ({
      ...prev,
      playlists: prev.playlists.map((pl) =>
        pl.id === playlistId
          ? { ...pl, name: nextName, updatedAt: new Date().toISOString() }
          : pl
      ),
    }));
    toast.success("Playlist updated");
  ...
  async function logout() {
  try {
    hasLoadedRef.current = false; // ← stop save effect before clearing library
    await logoutUser();
    setLibrary(defaultLibraryState);
    toast.success("Logged out");
  } catch {
  ...
  const value = {
    currentTrack, queue, currentIndex, isPlaying, volume, currentTime, duration,
    library, libraryReady, isCloudSyncEnabled, user, isLoggedIn,
    login, loginWithEmailPassword, register, logout,
    playQueue, playTrack, playNext, playPrevious,
    setPlaying, setVolume, setPlaybackTime, setPlaybackDuration,
    toggleLike, isTrackLiked, createPlaylist, addTrackToPlaylist,
    addTracksToPlaylist, renamePlaylist, deletePlaylist, saveJourney,
  };
